function currentWeather(data) {
  ...
  let date = data.current_observation.observation_time
  let high = todayWeather.high.fahrenheit
  let low = todayWeather.low.fahrenheit

  return { temp, location, textForecast, shortForecast, iconSrc, date, high, low }
}
...
function hourlyWeather(data) {
  ...
  return forecastArray.map(hour => {
    ...
    let iconSrc = hour.icon_url
    let temp = hour.temp.english
    let hourly = true;
    return { time, iconSrc, temp, hourly }
  }) 
}
...
function dailyWeather(data) {
  ...
  return forecastArray.map(day => {
    ...
    return { weekDay, iconSrc, high, low, daily }
  })
